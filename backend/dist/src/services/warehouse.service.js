...
exports.deactivateWarehouse = deactivateWarehouse;
exports.activateWarehouse = activateWarehouse;
...
async function createWarehouse(data) {
    return prisma.$transaction(async (tx) => {
        ...
        if (project.status !== "ACTIVE") {
            throw new Error("Cannot create warehouse under an inactive project");
        }
        // ==================================================
        ...
        if (data.responsibleUserId) {
            ...
            if (user.status !== "ACTIVE") {
                throw new Error("Responsible user is inactive");
            }
        }
        // ==================================================
        ...
        if (existing) {
            throw new Error(`Warehouse code already exists in this project: ${data.code}`);
        }
        // ==================================================
        // 4. Create warehouse
        // ==================================================
        const warehouse = await tx.warehouse.create({
            data: {
                ...
                responsibleUserId: data.responsibleUserId ??
                    null,
                capacityDescription: data.capacityDescription ??
                    null,
 ...
 * Get Warehouses
 */
async function getWarehouses(options = {}) {
...
async function updateWarehouse(id, data) {
    return prisma.$transaction(async (tx) => {
        ...
        if (project.status !== "ACTIVE") {
            throw new Error("Cannot assign warehouse to an inactive project");
        }
        // ==================================================
        ...
        if (duplicate) {
            throw new Error(`Warehouse code already exists in this project: ${code}`);
        }
        // ==================================================
        // 5. Build update data
        // ==================================================
...
async function deactivateWarehouse(id) {
    return prisma.$transaction(async (tx) => {
        ...
        // ================================================
        const activeLocations = warehouse.storageLocations.filter((location) => location.isActive);
        if (activeLocations.length > 0) {
            throw new Error("Cannot deactivate warehouse while active storage locations exist");
        }
        return tx.warehouse.update({
            where: {
                id,
...
async function activateWarehouse(id) {
    return prisma.$transaction(async (tx) => {
        ...
        if (warehouse.project.status !== "ACTIVE") {
            throw new Error("Cannot activate warehouse under an inactive project");
        }
        return tx.warehouse.update({
            where: {
                id,
